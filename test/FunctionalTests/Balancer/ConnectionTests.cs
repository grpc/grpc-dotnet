...
#if !NET5_0_OR_GREATER
        [Test]
        public async Task Passive_PickFirst_MultipleStreams_UnavailableAddress_FallbackToWorkingAddress()
        {
            // Ignore errors
            SetExpectedErrorsFilter(writeContext =>
            {
            ...
            // Act
            var response1 = await client.UnaryCall(new HelloRequest { Name = "Balancer" }).ResponseAsync.DefaultTimeout();

            var balancer = BalancerHelpers.GetInnerLoadBalancer<PickFirstBalancer>(channel)!;
            var subchannel = balancer._subchannel!;
            var transport = (PassiveSubchannelTransport)subchannel.Transport;
            ...
            var callTasks = new List<Task<HelloReply>>();
            for (var i = 0; i < 5; i++)
            {
                Logger.LogInformation($"Sending gRPC call {i}");
            ...
            foreach (var callTask in callTasks)
            {
                var ex = await ExceptionAssert.ThrowsAsync<RpcException>(() => callTask);
            }

            var response2 = await client.UnaryCall(new HelloRequest { Name = "Balancer" }).ResponseAsync.DefaultTimeout();
            Assert.AreEqual("127.0.0.1:50052", host);
        }

        [Test]
        [Ignore("Work in progress.")]
        public async Task Passive_RoundRobin_MultipleStreams_UnavailableAddress_FallbackToWorkingAddress()
        {
            // Ignore errors
            SetExpectedErrorsFilter(writeContext =>
            {
            ...
            for (var i = 0; i < 10; i++)
            {
                var callTask = client.UnaryCall(new HelloRequest { Name = "Balancer" }).ResponseAsync;
                if (i % 2 == 0)
                {
            ...
            //var balancer = BalancerHelpers.GetInnerLoadBalancer<RoundRobinBalancer>(channel)!;
            //var subchannel = balancer.!;
            //var transport = (PassiveSubchannelTransport)subchannel.Transport;
