...
namespace Grpc.Net.Client.Tests
{
    ...
    public class GrpcChannelTests
    {
        ...
        public void Build_NoHttpProviderOnNetFx_Throw()
        {
            ...
            // Assert
            var message =
                $"gRPC requires extra configuration to successfully make RPC calls on .NET implementations that don't " +
                $"have support for gRPC over HTTP/2. An HTTP provider must be specified using {nameof(GrpcChannelOptions)}.{nameof(GrpcChannelOptions.HttpHandler)}. " +
                $"The configured HTTP provider must either support HTTP/2 or be configured to use gRPC-Web. See https://aka.ms/pzkMXDs for details.";
